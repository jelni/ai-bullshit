...
fn handle_menu_input(code: KeyCode, game: &mut Game) -> bool {
    match code {
        ...
        KeyCode::Char(' ') | KeyCode::Enter => match game.menu_selection {
            ...
            47 => game.state = GameState::Settings,
            48 => game.state = GameState::NftShop,
            49 => game.state = GameState::SkillTree,
            50 => game.state = GameState::Stats,
            51 => game.state = GameState::Achievements,
            52 => game.state = GameState::Help,
            53 => {
        ...
        KeyCode::Up | KeyCode::Char('w' | 'W') => {
            if game.menu_selection > 0 {
                ...
                game.menu_selection = 77;
            }
        },
        KeyCode::Down | KeyCode::Char('s' | 'S') => {
            if game.menu_selection < 77 {
                game.menu_selection += 1;
            } else {
...
#[expect(clippy::too_many_lines, reason = "This is a big match statement")]
    ...
    match code {
        ...
        KeyCode::Up | KeyCode::Char('w' | 'W') => {
            if game.settings_selection > 0 {
                ...
                game.settings_selection = 9;
            }
        },
        KeyCode::Down | KeyCode::Char('s' | 'S') => {
            if game.settings_selection < 9 {
                game.settings_selection += 1;
            } else {
                game.settings_selection = 0;
            }
        ...
        KeyCode::Enter | KeyCode::Char(' ') => match game.settings_selection {
            ...
            7 => {
                if game.stats.unlocked_classes.contains(&crate::game::HeroClass::Ninja) {
                    ...
                    game.stats.coins -= 500;
                    game.stats.unlocked_classes.push(crate::game::HeroClass::Ninja);
                    game.stats.equipped_class = Some(crate::game::HeroClass::Ninja);
                }
            },
            8 => {
                if game.stats.unlocked_classes.contains(&crate::game::HeroClass::Druid) {
                    ...
                    game.stats.coins -= 500;
                    game.stats.unlocked_classes.push(crate::game::HeroClass::Druid);
                    game.stats.equipped_class = Some(crate::game::HeroClass::Druid);
                }
            },
            9 => {
                if game.stats.unlocked_classes.contains(&crate::game::HeroClass::Engineer) {
                    ...
                    game.stats.coins -= 500;
                    game.stats.unlocked_classes.push(crate::game::HeroClass::Engineer);
                    game.stats.equipped_class = Some(crate::game::HeroClass::Engineer);
                }
            },
            10 => {
