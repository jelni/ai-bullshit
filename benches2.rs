...
fn generate_food_hashset(width: u16, height: u16, snake: &Snake, obstacles: &[Point]) -> Point {
    let mut set = HashSet::with_capacity(snake.body.len() + obstacles.len());
    for p in &snake.body {
        set.insert(*p);
    }
    for p in obstacles {
...
fn main() {
    ...
    let start = std::time::Instant::now();
    for _ in 0..100 {
        let p = generate_food_hashset(width, height, &snake, &[]);
        std::hint::black_box(p);
    }
    println!("HashSet took: {:?}", start.elapsed());
