...
fn test_time_freeze_pauses_boss_and_lasers() {
    ...
    let boss_initial_pos = snake::Point { x: 10, y: 10 };
    ...
    let laser_initial_pos = snake::Point { x: 5, y: 5 };
    ...
    // Put snake far away
    game.snake = snake::Snake::new(snake::Point { x: 18, y: 18 });
    ...
    assert_eq!(game.lasers[0].position, laser_initial_pos, "Laser should not move while time is frozen");
}
...
fn test_bot_avoids_frozen_boss() {
    ...
    game.food = snake::Point { x: 9, y: 5 };

    // Placing boss right in front of the snake
    game.boss = Some(game::Boss {
        position: snake::Point { x: 6, y: 5 },
        health: 10,
        max_health: 10,
        move_timer: 0,
        shoot_timer: 0,
    ...
    assert!(next_move == Some(snake::Direction::Up) || next_move == Some(snake::Direction::Down), "Bot should pathfind around the frozen boss");
}
...
fn test_bot_avoids_frozen_laser() {
    ...
    let next_move = game.calculate_autopilot_move();
    assert!(next_move == Some(snake::Direction::Up) || next_move == Some(snake::Direction::Down), "Bot should pathfind around the frozen laser");
}
