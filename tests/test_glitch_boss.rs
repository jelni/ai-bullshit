...
fn test_glitch_boss_scrambles_controls() {
    ...
    assert_eq!(
        game.controls_scrambled_timer, 50,
        "Glitch boss should set controls_scrambled_timer to 50"
    );
    ...
    assert_eq!(
        game.snake.direction_queue.front().copied(),
        Some(Direction::Left),
        "Input should be scrambled from Right to Left"
    );

    game.handle_input(Direction::Up, 1);
    assert_eq!(game.snake.direction_queue.len(), 2);
    assert_eq!(
        game.snake.direction_queue.back().copied(),
        Some(Direction::Down),
        "Input should be scrambled from Up to Down"
    );
