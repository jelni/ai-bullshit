...
fn test_boss_blocks_all_paths() {
    ...
    game.food = snake::Point { x: 18, y: 18 }; // Food is far away

    // Boss is at x=10, y=10
    game.boss = Some(game::Boss {
        position: snake::Point { x: 10, y: 10 },
        health: 10,
        max_health: 10,
        move_timer: 0,
        shoot_timer: 0,
    ...
    assert!(next_move.is_some(), "Autopilot failed to find a path because boss blocked everything!");
}
