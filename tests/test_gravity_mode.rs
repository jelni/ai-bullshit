use snake_game::game::{Game, GameMode, GameState, Difficulty, Theme};
...
fn test_gravity_mode() {
    ...
    game.snake.body.push_back(Point { x: 10, y: 10 });
    game.snake.body_map.clear();
    game.snake.body_map.insert(Point { x: 10, y: 10 }, 1);
    ...
    for _ in 0..100 {
        ...
        if game.snake.head().x >= 18 {
            game.snake.body.clear();
            game.snake.body.push_back(Point { x: 5, y: new_y });
            game.snake.body_map.clear();
            game.snake.body_map.insert(Point { x: 5, y: new_y }, 1);
        }
    }
