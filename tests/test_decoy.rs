...
fn test_boss_targets_decoy() {
    ...
    let initial_dist_to_decoy =
        game.boss.as_ref().unwrap().position.x.abs_diff(game.decoy.unwrap().0.x)
            + game.boss.as_ref().unwrap().position.y.abs_diff(game.decoy.unwrap().0.y);

    // Play a frame
    game.state = snake_game::game::GameState::Playing;
    game.update();

    let new_boss_pos = game.boss.as_ref().unwrap().position;
